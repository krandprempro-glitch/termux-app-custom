...
import com.linux.terminal.shared.reflection.ReflectionUtils;
...
import com.linux.terminal.shared.termux.TermuxConstants.TERMUX_APP;
...
public class TermuxUtils {
     ...
     *
     * If your third-party app is targeting sdk `30` (android `11`), then it needs to add `com.linux.terminal`
     * package to the `queries` element or request `QUERY_ALL_PACKAGES` permission in its
     * `AndroidManifest.xml`. Otherwise it will get `PackageSetting{...... com.linux.terminal/......} BLOCKED`
     ...
     * <manifest
     *     <queries>
     *         <package android:name="com.linux.terminal" />
     *    </queries>
     * </manifest>
     * }
     *
    ...
    public static String geAPTInfoMarkdownString(@NonNull final Context context) {
        ...
        InputStream inputStream = context.getResources().openRawResource(com.linux.terminal.shared.R.raw.apt_info_script);
        try {
